...
The user interface is divided into three main sections: the landing page and two lateral sections.

- **Landing Page**: Offers a split view of the model stored in Speckle and the model generated by
  Rhino Compute. Users can interact with the model by adjusting parameters and viewing the updated
  results in real-time.

- **Speckle Optimal Commit**: Displays metadata associated with the model, allowing users to select
  commits filtered based on their chosen parameters.

- **Speckle Parts**: Shows the individual components of the model, including counts of parts per
  group. It provides detailed information related to each part, such as cost, material, and
  lifecycle data.
